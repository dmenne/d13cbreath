...
ComputeAndSaveParameterizedFit = function(con,BreathTestRecordID)  {
  ...
  if (inherits("data","try-error")  |
      nrow(data) == 0 | all(data$PDR == 0))
    stop(
      paste(
        "No PDR data found for BreathTestRecordID; did you supply weight and height?",BreathTestRecordID
      )
    )
  ...
  start = list(m = sum(data$PDR * diff(c(0,data$Time))) / 70, # Empirical
               k = 1 / 100,beta = 2)
  bid.nls = try(suppressWarnings(nls(
    PDR ~ ExpBeta(Time,Dose,m,k,beta),
  ...
  # Delete old values
  methods = c(
    "ExpBeta","ExpBeta","ExpBeta","ExpBeta","BluckCoward","Maes","MaesScint",
    "BluckCoward","Maes"
  )
